...
path.append('/home/sulei/anaconda3/envs/forcaffe1/bin/python')
import argparse
...
class ConvertBnn:
    ...
    def search_bottom_blob(self, target_name):
        layer_params = self.net_param.layer
        length = len(layer_params)
        i = 0
        total = 0
        while i < length:
            current_bottoms = layer_params[i].bottom
            for item in current_bottoms:
                if (item == target_name and not item == layer_params[i].top[0]):
                    total = total + 1
                    if (total >= 2):
                        return True
            i = i + 1
    ...
    def pre_process(self):
        ...
        while i < length:
            ...
            if layer_params[i].type in layer_type:
                multi_branch_or_not = False
                for top_item in layer_params[i].top:
                    multi_branch_or_not = multi_branch_or_not or self.search_bottom_blob(top_item)
                if ((i + 2 < length) and layer_params[i + 1].type == bnn_type[0] and  \
                    layer_params[i + 2].type == bnn_type[1] and not multi_branch_or_not):
                    ...
                    i=i+1
            elif layer_params[i].type == 'Scale' and layer_params[i-1].type == bnn_type[0] and not layer_params[i-2].type in layer_type:
                layer_params[i].scale_param.bias_term = True;
                layer_params[i].scale_param.bias_filler.type = 'constant'
                layer_params[i].scale_param.bias_filler.value = 0
                self.bnn_layer_location.extend([i])
        ...
        for i, layer in enumerate(model_layers):
            ...
            for index_num in range(len(layer.blobs)):
                self.dest_model.layers[i].blobs[index_num] = layer.blobs[index_num]
        print('asdf end')
    ...
    def convert(self):
        ...
        while l < length:
            ...
            if param_layers[i].type in layer_type:
                ...
                if (len(self.net_model.params[param_layers[i+2].name]) > 1):
                ...
                for k in range(channels):
                    ...
                    if (len(self.dest_model.params[param_layers[i].name]) > 1):
            ...
            elif param_layers[i].type == 'Scale' and param_layers[i-1].type == 'BatchNorm' and not param_layers[i-2].type in layer_type:
                ...
                if (len(self.net_model.params[param_layers[i].name]) > 1):
                    b = copy.deepcopy(self.net_model.params[param_layers[i].name][1].data)
                else:
                    b = np.zeros(channels)
                for k in range(channels):
                    self.dest_model.params[param_layers[i].name][0].data[k] = 1.000 * a[k] / std[k]
                    if (len(self.dest_model.params[param_layers[i].name]) > 1):
                        self.dest_model.params[param_layers[i].name][1].data[k] =1.000 * a[k] * (1.0000000000 - mean[k] ) / std[k] + b[k]
                for bottom_index in range(len(param_layers[i-1].bottom)):
                    param_layers[i].bottom[bottom_index] = self.net_param.layer[i - 1].bottom[bottom_index]
            l += 1
